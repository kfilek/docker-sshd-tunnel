...
if [[ "${SSH_ENABLE_ROOT}" == "true" ]]; then
    ...
    usermod -p '' root
    echo "set /files/etc/ssh/sshd_config/PermitRootLogin prohibit-password" | augtool -s 1> /dev/null
else
    echo "INFO: root account is now locked by default. Set SSH_ENABLE_ROOT to unlock the account."
    echo "set /files/etc/ssh/sshd_config/PermitRootLogin no" | augtool -s 1> /dev/null
fi
...
# Enable Logging
if [ -n "${LOG_LEVEL}" ]; then
    echo "set /files/etc/ssh/sshd_config/LogLevel ${LOG_LEVEL}" | augtool -s 1> /dev/null
fi
...
if [[ "${SFTP_MODE}" == "true" ]]; then
  ...
  echo "INFO: configuring rsync only mode"
    ...
    if [[ "${TCP_FORWARDING}" == "true" ]]; then
        echo 'set /files/etc/ssh/sshd_config/AllowTcpForwarding all' | augtool -s 1> /dev/null
    fi
    # Enable GatewayPorts
